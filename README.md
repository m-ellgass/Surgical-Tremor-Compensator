...
## Future Work
Tremor filtering is a common feature of medical robotic and assistive devices, and in many applications, such as millimeter-level precision surgery, the filter must distinguish between intentional small-scale movement and unintentional shakes, jerks, or tremors. In this tremor compensator project, a simple complementary EMA filter was used to remove such noise, and it does so relatively well. However, the reason it works well is due to the predictability of the accelerometer and gyroscope behavior. The noise level of the accelerometer and the drifting of the gyroscope are fairly constant, so relying on a hard-coded 98% gyro to 2% accelerometer works. But in the real world, and especially when such fine-tuned precision is needed, a static ratio of sensor input isn’t ideal. This is where the Kalman Filter comes in. A Kalman Filter is an algorithm that estimates a true state given noisy signal inputs by blending them with a dynamic prediction model. At a high-level, a Kalman Filter is a more expensive but dynamic and adaptable version of the filter implemented in this project. Additionally, in a real-world surgical robot or adaptive prosthetic, it would be ideal to have more degrees of freedom. This project is strictly a one-dimensional model; it tracks pitch, rotation along one axis. A more complete implementation would track both pitch and roll, utilizing two simultaneous but separate complementary filters, and would require an additional servo. Tracking yaw with an IMU gets more difficult, since the accelerometer uses gravity, and gravitational forces on the chip would not change with a change in yaw, so to implement a third dimension, an additional IMU perpendicular to the first could be used.
